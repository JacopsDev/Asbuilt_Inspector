...
class AsbuiltInspector:
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir,
            'i18n',
            'AsbuiltInspector_{}.qm'.format(locale))
        ...
        self.actions = []
        self.menu = self.tr(u'&AsbuiltInspector')
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('AsbuiltInspector', message)
    ...
    def initGui(self):
        ...
        icon_patha = os.path.join(plugin_dir,
                                 'water-color.png')
        self.icon_active = QIcon(icon_patha)
        ...
        self.inspect_action = QAction(self.icon_inactive, self.tr(u'SABINese-tool'), self.iface.mainWindow())
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginMenu(
                self.tr(u'&AsbuiltInspector'),
                action)
            self.iface.removeToolBarIcon(action)

    def run(self):
        """Run method that performs all the real work"""
        if not hasattr(self, 'tool_active'):
            self.tool_active = False

        canvas = self.iface.mapCanvas()

        if not self.tool_active:
            self.dlg = AsbuiltInspectorDialog(self.iface)
            canvas.setMapTool(self.dlg)
